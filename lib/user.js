...
User.getByName = function(name, fn) {
    connection.query("SELECT user_id, name, salt, pass, group_name FROM users, user_groups " +
        "WHERE users.group_id=user_groups.group_id AND name=?", 
        [name], 
        function(err, rows){
            if (err) return fn(err);
...
User.getById = function(id, fn) {
    connection.query("SELECT user_id, name, group_name FROM users, user_groups " + 
        "WHERE users.group_id=user_groups.group_id AND id=?", 
        [id], 
        function(err, rows) {
            if (err) return fn(err);
            var user = new User(rows[0]);
